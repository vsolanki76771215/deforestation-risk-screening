"""Validate Task 3.14 precision-aware model artifacts and safeguards."""
...
def main() -> int:
    ...
    if missing:
        result = {"task": "3.14-precision-mitigation", "status": "FAILED", "checks": checks, "errors": errors}
    ...
    check("holdout_protocol", protocol.get("fit_split") == "train"
          and protocol.get("threshold_selection_split") == "validation"
          and protocol.get("test_used_for_selection") is False
          and protocol.get("final_holdout_used_for_selection") is False,
          "training and threshold selection must exclude test and final_holdout")
    selection = report.get("threshold_selection", {})
    check("precision_threshold_policy", "minimum_precision_constraint_met" in selection,
          "report must identify the validation precision policy")
    ...
    actual_splits = set(manifest[columns["split"]].astype(str))
    allowed_splits = required_splits | {"final_holdout"}
    check("split_schema", required_splits <= actual_splits and actual_splits <= allowed_splits,
    ...
    result = {
        "task": "3.14-precision-mitigation", "status": "PASSED" if not errors else "FAILED",
    ...
    print(f"Validation summary: {output_path}")
    print(f"Task 3.14 precision-aware validation: {result['status']}")

"""Train the Task 3.14 final precision-aware ExtraTrees model.

The final geographic holdout is loaded for one-time reporting only.  It is
never used for fitting, threshold selection, or quality-gate decisions.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="config/precision_aware_model_task_3_14.yaml")
...
def select_threshold(y_true: np.ndarray, probability: np.ndarray, minimum_precision: float, points: int) -> dict:
    if points < 2:
    ...
    # Threshold selection is restricted to validation.  Precision controls alert
    # volume; F1 balances precision and recall inside that operating envelope.
    eligible = [row for row in candidates if row[2] >= minimum_precision]
    ...
    return {
        ...
        "minimum_precision_constraint_met": bool(eligible),
    }
...
def main() -> int:
    ...
    if existing and not args.overwrite:
        raise FileExistsError(f"Task 3.14 precision-aware output exists; pass --overwrite to replace it: {existing}")
    ...
    allowed_splits = required_splits | {"final_holdout"}
    if not required_splits <= actual_splits or not actual_splits <= allowed_splits:
        raise ValueError(
            "Expected train/validation/test with an optional final_holdout split, "
    ...
    if "final_holdout" in actual_splits:
        split_order.append("final_holdout")
    masks = {name: frame[split_column].eq(name).to_numpy() for name in split_order}
    y = target.to_numpy()
    model_cfg = config["model"]
    if model_cfg["type"] != "ExtraTreesClassifier":
        raise ValueError("This workflow supports ExtraTreesClassifier")
    ...
    print(f"Training Extra Trees on {int(masks['train'].sum())} patches...", flush=True)
    ...
    threshold_result = select_threshold(
        ...
        float(config["threshold_selection"]["minimum_precision"]),
    ...
    result = {
        "task": "3.14-precision-mitigation", "status": "COMPLETED", "model_type": model_cfg["type"],
        ...
        "protocol": {
            "fit_split": "train", "threshold_selection_split": "validation",
            "test_used_for_selection": False, "test_evaluations_after_selection": 1,
            "final_holdout_used_for_selection": False, "final_holdout_evaluations_after_selection": 1,
        },
    ...
    print("Task 3.14 precision-aware mitigation: COMPLETED")
    return 0
